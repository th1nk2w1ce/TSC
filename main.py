...
async def start_command(message: types.Message):
        ...
        if len(message.text.split()) > 1:
            if int(message.text.split()[1]) != int(message.from_user.id) and cur.execute(f"SELECT tg_id FROM users WHERE tg_id == {message.text.split()[1]}").fetchall():
                try:
                    ...
                    connector_referer = TonConnect(manifest_url='https://raw.githubusercontent.com/coinvent-solutions/TSPC-public/main/manifest2.json', storage=storage_referer)
    ...
    connector = TonConnect(manifest_url='https://raw.githubusercontent.com/coinvent-solutions/TSPC-public/main/manifest2.json', storage=storage)
    # Attempt to restore the existing connection, if any
